    ...
    if (empty($errors)) {
        if ($newPass !== '') {
            ...
            $pdo->prepare('UPDATE users SET name=?, cat_name=?, phone=?, address=?, password=? WHERE id=?')
                ->execute([$name, $cat_name, $phone, $address, $hashed, $user['id']]);
        } else {
...
$orderStmt = $pdo->prepare('SELECT o.*, COUNT(oi.id) AS item_count FROM orders o
...
<section style="padding:60px 5%;min-height:70vh;">
  <div style="max-width:820px;margin:0 auto;">
    ...
    <div style="background:var(--brown);border-radius:28px;padding:24px 36px;display:flex;align-items:center;gap:20px;margin-bottom:32px;flex-wrap:wrap;">
      <div style="width:68px;height:68px;border-radius:50%;background:var(--orange);display:flex;align-items:center;justify-content:center;font-size:2.2rem;flex-shrink:0;">🐱</div>
      <div style="flex:1;">
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;">
        <a href="?tab=profile" style="background:<?= $tab==='profile'?'var(--orange)':'rgba(255,255,255,.12)' ?>;color:var(--cream);border-radius:20px;padding:8px 20px;font-size:.82rem;font-weight:600;text-decoration:none;">Profile</a>
        <a href="?tab=orders"  style="background:<?= $tab==='orders'?'var(--orange)':'rgba(255,255,255,.12)' ?>;color:var(--cream);border-radius:20px;padding:8px 20px;font-size:.82rem;font-weight:600;text-decoration:none;">My Orders (<?= count($orders) ?>)</a>
      </div>
    </div>
    ...
    <div class="membership-right" style="background:var(--white);border:1.5px solid var(--warm);">
      ...
      <form method="POST" novalidate>
        ...
        <div class="form-field">
          <label for="pf-name" style="color:var(--brown-md);">Full Name</label>
        ...
        <div class="form-field">
          <label for="pf-cat" style="color:var(--brown-md);">Your Cat's Name 🐱</label>
          <input id="pf-cat" type="text" name="cat_name" value="<?= h($profile['cat_name'] ?? '') ?>" placeholder="e.g. Mochi"
                 style="background:var(--warm);border-color:var(--warm);color:var(--brown);"/>
        </div>
        <div class="form-field">
          <label for="pf-phone" style="color:var(--brown-md);">Phone Number</label>
          <input id="pf-phone" type="tel" name="phone" value="<?= h($profile['phone'] ?? '') ?>" placeholder="+65 9123 4567"
                 style="background:var(--warm);border-color:var(--warm);color:var(--brown);"/>
        </div>
        <div class="form-field">
          <label for="pf-addr" style="color:var(--brown-md);">Delivery Address</label>
          <input id="pf-addr" type="text" name="address" value="<?= h($profile['address'] ?? '') ?>" placeholder="12 Whisker Lane, Singapore"
                 style="background:var(--warm);border-color:var(--warm);color:var(--brown);"/>
        </div>
        <hr style="border:none;border-top:1.5px solid var(--warm);margin:24px 0;">
        <h4 style="color:var(--brown-md);font-size:.9rem;margin-bottom:16px;">Change Password <span style="font-weight:400;font-size:.8rem;">(leave blank to keep current)</span></h4>
        <div class="form-field">
        ...
        <button class="btn-primary" type="submit" style="width:100%;display:block;text-align:center;">Save Changes</button>
      </form>
    </div>
    ...
    <?php if (empty($orders)): ?>
      <div style="background:var(--white);border-radius:20px;padding:60px 24px;text-align:center;border:1.5px solid var(--warm);">
        ...
        <p style="color:var(--brown-md);margin-bottom:20px;">You haven't placed any orders yet.</p>
      ...
      ?>
        <div style="background:var(--white);border-radius:20px;margin-bottom:16px;border:1.5px solid var(--warm);overflow:hidden;">
          <div style="padding:14px 22px;background:var(--warm);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
            ...
            <span style="background:<?= $sc['bg'] ?>;color:<?= $sc['color'] ?>;border-radius:16px;padding:3px 12px;font-size:.75rem;font-weight:700;">
              <?= h($sc['label']) ?>
            </span>
          </div>
