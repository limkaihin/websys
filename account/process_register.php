...
function saveMemberToDB()
{
    ...
    $stmt = $conn->prepare(
        "INSERT INTO meowmart_members (fname, lname, email, cat_name, password)
         VALUES (?, ?, ?, ?, ?)"
    );
    ...
    if (!$stmt->execute()) {
        if ($stmt->errno == 1062) {
            ...
            $errorMsg = "Error: (" . $stmt->errno . ") " . $stmt->error;
        }
        $success = false;
    }
...
$fname = $lname = $email = $cat_name = $errorMsg = "";
$pwd   = $pwd_confirm = "";
$success = true;
...
<head>
  ...
  <title>Registration Result – MeowMart</title>
</head>
<body>
  ...
  <div style="max-width:500px;width:100%;text-align:center;">
    <?php if ($success): ?>
      <div style="background:var(--warm);border-radius:28px;padding:48px 40px;">
        ...
        <p style="color:var(--brown-md);margin-bottom:8px;">
          Thank you for signing up,
          <strong><?= htmlspecialchars($fname) . " " . htmlspecialchars($lname) ?></strong>!
        </p>
        <?php if ($cat_name): ?>
          <p style="color:var(--orange);font-size:.95rem;margin-bottom:28px;">
            🐱 Say hi to <?= htmlspecialchars($cat_name) ?> for us!
          </p>
        <?php else: ?>
    ...
    <?php else: ?>
      <div style="background:var(--brown);border-radius:28px;padding:48px 40px;">
        ...
        <h2 style="font-family:'Playfair Display',serif;font-size:1.8rem;margin-bottom:12px;color:var(--cream);">
          Oops! Please fix these:
        </h2>
        <p style="color:var(--blush);margin-bottom:28px;text-align:left;line-height:1.8;">
          <?= $errorMsg ?>
        </p>
        <a href="register.php" class="btn-join" style="text-decoration:none;display:inline-block;">
          Go Back →
        </a>
      </div>
    <?php endif; ?>
